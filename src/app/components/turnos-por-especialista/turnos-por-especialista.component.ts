...
export class TurnosPorEspecialistaComponent{
  ...
  async loadData() {
    try {
      ...
      {
        const data = await this.dataService.getCantidadTurnosPorEspecialista(this.fechaInicio, this.fechaFin, false);
        ...
        this.chartOption = {
          title: {
            text: 'Cantidad de Turnos NO FINALIZADOS por Especialista'
          },
          tooltip: {
            trigger: 'axis',
            axisPointer: {
